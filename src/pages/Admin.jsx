  ...
  useEffect(() => {
    const verify = async () => {
      ...
      try {
        ...
        if (me?.isAdmin) setAuthState('admin');
  ...
  if (authState === 'anonymous') {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-text-muted text-sm mb-6">Please log in to access the admin dashboard.</p>
        <Link to="/login" className="inline-block px-6 py-3 bg-accent-blue text-white text-xs font-bold rounded-xl">
          Go to Login
        </Link>
      </div>
    );
