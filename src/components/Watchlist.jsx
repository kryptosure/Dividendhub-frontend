...
const Watchlist = () => {
  ...
  if (watchlist.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-border/40 rounded-2xl bg-bg-surface/30 max-w-xl mx-auto mt-6">
        ...
        <p className="text-text-muted text-xs font-medium mt-1 max-w-xs">Add stocks from the search page to track them here.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-0 mt-4 animate-in fade-in duration-200">
      <h1 className="text-2xl font-black text-text-primary tracking-tight mb-6">Watchlist</h1>
        ...
        {watchlist.map((item) => (
          ...
          >
            <div className="min-w-0 pr-4">
              ...
              <p className="text-xs font-mono font-bold text-accent-teal mt-0.5 tracking-wider uppercase">{item.symbol} • {item.market || 'US'}</p>
            </div>
            <button
              onClick={(e) => {
                e.stopPropagation();
