...
const ShareCardModal = ({ isOpen, onClose, metrics, monthlyAmount, drip, yearsToTarget, finalValue, currencySymbol = '$' }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200 overflow-y-auto" onClick={onClose}>
      <div className="max-w-lg w-full space-y-3 my-auto" onClick={(e) => e.stopPropagation()}>
        ...
        >
          {/* Brand header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: 'clamp(16px, 4vw, 28px)' }}>
            <div style={{
          ...
          <div style={{ marginBottom: 'clamp(14px, 3vw, 20px)' }}>
            <div style={{ fontSize: '10px', color: '#94a3b8', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            ...
            <div style={{
              ...
              color: '#06b6d4', letterSpacing: '0.05em',
            }}>{metrics.symbol}</div>
          </div>
          ...
          <div className="modal-compact-stack" style={{
            display: 'grid',
            gridTemplateColumns: '1fr 1fr 1fr',
            gap: '10px',
            marginBottom: 'clamp(16px, 4vw, 24px)',
          }}>
            <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '10px', padding: '10px 8px', minWidth: 0 }}>
              <div style={{ fontSize: '9px', color: '#64748b', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Monthly</div>
              <div style={{ fontSize: 'clamp(13px, 3.5vw, 16px)', fontWeight: 900, marginTop: '4px', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            ...
            <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '10px', padding: '10px 8px', minWidth: 0 }}>
              ...
              <div style={{ fontSize: 'clamp(13px, 3.5vw, 16px)', fontWeight: 900, marginTop: '4px', fontFamily: 'monospace', color: '#10b981' }}>
            ...
            <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '10px', padding: '10px 8px', minWidth: 0 }}>
              ...
              <div style={{ fontSize: 'clamp(13px, 3.5vw, 16px)', fontWeight: 900, marginTop: '4px', fontFamily: 'monospace', color: drip ? '#06b6d4' : '#94a3b8' }}>
                {drip ? 'ON' : 'OFF'}
              </div>
            </div>
          </div>
          ...
          <div style={{
            ...
            gap: '8px', paddingTop: 'clamp(10px, 2.5vw, 16px)',
            borderTop: '1px solid #1e293b', flexWrap: 'wrap',
          }}>
            <div style={{ fontSize: '10px', color: '#64748b', fontWeight: 600 }}>Simulate yours free →</div>
            <div style={{ fontSize: '11px', fontWeight: 800, color: '#06b6d4', letterSpacing: '-0.01em' }}>
              dividendbro.com/millionaire
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="px-3 py-3 bg-gradient-to-r from-accent-blue to-accent-teal text-white text-[11px] sm:text-xs font-bold uppercase tracking-wider rounded-xl shadow-md hover:opacity-95 active:scale-95 disabled:opacity-50 transition-all"
          >
            {isGenerating ? 'Generating...' : '⬇️ Download'}
          </button>
          <button
            onClick={handleWhatsApp}
            className="px-3 py-3 bg-[#25D366]/10 border border-[#25D366]/30 text-[#25D366] text-[11px] sm:text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-[#25D366]/20 active:scale-95 transition-all"
          >
            💬 WhatsApp
          </button>
          <button
            onClick={handleTweet}
            className="px-3 py-3 bg-bg-surface border border-border/60 text-text-primary text-[11px] sm:text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-bg-surface-hover active:scale-95 transition-all"
          >
            🐦 Post to X
          </button>
          <button
