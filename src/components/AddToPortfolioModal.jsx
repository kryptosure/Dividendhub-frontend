  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-bg-secondary border border-border/80 rounded-2xl max-w-md w-full p-6 shadow-xl animate-in zoom-in-95 duration-200">
        <div className="mb-4">
          <h3 className="text-xl font-black text-text-primary tracking-tight">Add Investment To My Portfolio</h3>
          <p className="text-text-muted text-xs font-medium mt-0.5">Adding <span className="text-text-primary font-bold">{name || symbol}</span> ({symbol}) to my portfolio.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {/* Changed bg-bg-primary/50 to bg-bg-secondary */}
            <input
              type="number"
              value={quantity}
              ...
              min="1"
              step="1"
              className="w-full bg-bg-secondary border border-border/60 rounded-xl px-4 py-2.5 text-sm font-medium text-text-primary focus:outline-none focus:border-accent-blue focus:ring-4 focus:ring-accent-blue/5"
              required
            />
          </div>
